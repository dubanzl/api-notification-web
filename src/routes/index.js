    ...
    const [rows, fields] = await connection.execute('select * from notification_suscribe where project = ? and id_user = ?', [data.project, data.idUser]);
      if(rows.length > 0){
          rows.map((value) =>  {
            ...
            .catch(function (ex) {
             await connection.execute('delete from notification_suscribe where project = ? and id_user = ? and p256dh = ? and auth = ?', [value.project, value.idUser, value.p256dh, value.auth ]);
          }
        );
      });
